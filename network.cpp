...
BOOL NetworkStartup()
   ...
   hWinSockDLL = LoadLibrary((LPCWSTR) "WSOCK32.DLL");
   
   if (hWinSockDLL == NULL)
      {
      ::MessageBox(::GetFocus(), L"Cannot load WSOCK32.DLL", L"Network Error", MB_OK | MB_ICONEXCLAMATION | MB_TASKMODAL);
      return FALSE;
      }
   ...
   if (lpWSAStartup(MAKEWORD(1,1), &WSAData) != 0)
      {
		  ::MessageBox(::GetFocus(), CA2CT(WSAGetLastErrorString(0)), L"WSAStartup()", MB_OK | MB_ICONEXCLAMATION | MB_TASKMODAL);
      return FALSE;
      }
