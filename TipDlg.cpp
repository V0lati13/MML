...
: CDialog(IDD_TIP, pParent)
   ...
   // Now try to open the tips file (relative to EXE's directory)
   CString exeFile;
   GetModuleFileName(NULL,exeFile.GetBufferSetLength(MAX_PATH), MAX_PATH);
   exeFile.ReleaseBuffer();

   CString tipsFile = exeFile.Left(exeFile.ReverseFind('\\') + 1);
   tipsFile += "tips.txt";

   m_pStream = fopen(CStringA(tipsFile), "r"); // try EXE directory first
   if (m_pStream == NULL)
     m_pStream = fopen("tips.txt", "r"); // try working directory next
...
void CTipDlg::OnPaint()
   ...
   CWnd* pStatic = GetDlgItem(IDC_TIP_FRAME);
   CRect rect;
